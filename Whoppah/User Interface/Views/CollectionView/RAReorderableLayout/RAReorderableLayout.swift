...
public extension RAReorderableLayoutDataSource {
    func collectionView(_: UICollectionView, reorderingItemAlphaInSection _: Int) -> CGFloat {
        0
    }

    func scrollTrigerEdgeInsetsInCollectionView(_: UICollectionView) -> UIEdgeInsets {
        .init(top: 100, left: 100, bottom: 100, right: 100)
    }

    func scrollTrigerPaddingInCollectionView(_: UICollectionView) -> UIEdgeInsets {
        .zero
    }

    func scrollSpeedValueInCollectionView(_: UICollectionView) -> CGFloat {
        10
    }
}

public extension RAReorderableLayoutDelegate {
    func collectionView(_: UICollectionView, at _: IndexPath, willMoveTo _: IndexPath) {}
    func collectionView(_: UICollectionView, at _: IndexPath, didMoveTo _: IndexPath) {}
    func collectionView(_: UICollectionView, allowMoveAt _: IndexPath) -> Bool {
        true
    }

    func collectionView(_: UICollectionView, at _: IndexPath, canMoveTo _: IndexPath) -> Bool {
        true
    }

    func collectionView(_: UICollectionView, collectionView _: RAReorderableLayout, willBeginDraggingItemAt _: IndexPath) {}
    func collectionView(_: UICollectionView, collectionView _: RAReorderableLayout, didBeginDraggingItemAt _: IndexPath) {}
    func collectionView(_: UICollectionView, collectionView _: RAReorderableLayout, willEndDraggingItemTo _: IndexPath) {}
...
open class RAReorderableLayout: UICollectionViewFlowLayout, UIGestureRecognizerDelegate {
    ...
    public weak var delegate: RAReorderableLayoutDelegate? {
        get { collectionView?.delegate as? RAReorderableLayoutDelegate }
        set { collectionView?.delegate = newValue }
    }

    public weak var dataSource: RAReorderableLayoutDataSource? {
        get { collectionView?.dataSource as? RAReorderableLayoutDataSource }
    ...
    fileprivate var triggerInsetTop: CGFloat {
        scrollDirection == .vertical ? trigerInsets.top : trigerInsets.left
    }
    ...
    fileprivate var triggerPaddingTop: CGFloat {
        scrollDirection == .vertical ? trigerPadding.top : trigerPadding.left
    }

    fileprivate var triggerPaddingEnd: CGFloat {
        scrollDirection == .vertical ? trigerPadding.bottom : trigerPadding.right
    }
    ...
    open override func prepare() {
        ...
        if let insets = dataSource?.scrollTrigerEdgeInsetsInCollectionView(collectionView!) {
            trigerInsets = insets
        }

        // scroll trier padding
        if let padding = dataSource?.scrollTrigerPaddingInCollectionView(collectionView!) {
    ...
    fileprivate func cancelDrag(_ toIndexPath: IndexPath!) {
        ...
        // will end drag item
        delegate?.collectionView(collectionView!, collectionView: self, willEndDraggingItemTo: toIndexPath)
...
private class RACellFakeView: UIView {
    ...
    init(cell: UICollectionViewCell) {
        ...
        cellFakeImageView = UIImageView(frame: bounds)
        ...
        cellFakeHightedView = UIImageView(frame: bounds)
