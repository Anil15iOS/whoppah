...
class AttributeBlockView: UIView {
    ...
    var onPageSelected: ((AttributeBlockSectionViewModel) -> Void)?
    ...
    private func commonInit() {
        Bundle.main.loadNibNamed("AttributeBlockView", owner: self, options: nil)
    ...
    func configure(withVM vm: AttributeBlockViewModel) {
        viewModel = vm
        bag = DisposeBag()
        vm.title.map { $0 == nil }.bind(to: titleContainer.rx.isHidden).disposed(by: bag)
    ...
    private func getSectionView(withVM vm: AttributeBlockSectionViewModel) -> AttributeBlockSectionView {
        let view = AttributeBlockSectionView(frame: CGRect(x: 0, y: 0, width: frame.width, height: frame.height))
        view.configure(withVM: vm)
        vm.outputs.objectClick.bind(to: viewModel.outputs.sectionClicked).disposed(by: bag)
        return view
    }
}
