...
open class IBANtools: NSObject {
  ...
  open class func isValidBIC(_ text: String?) -> Bool {
    ...
    return patternForBIC?.numberOfMatches(in: text!, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSRange(location: 0, length: (text!).count)) == 1
  }
  ...
  ///   "bic" (String)
  ///   "result" (IBANToolsResult in an NSNumber)
  open class func bicForBankCode(_ bankCode: String, countryCode: String) -> [String: AnyObject] {
  ...
  ///   "bic" (String)
  ///   "result" (IBANToolsResult in an NSNumber)
  open class func bicForIBAN(_ iban: String?) -> [String: AnyObject] {
  ...
  open class func bicForIBAN(_ iban: String?) -> (bic: String, result: IBANToolsResult) {
    ...
    if iban != nil && (iban!).count > 8 {
      ...
      if let details = countryData[countryCode.uppercased()] {
        let bankCode = (iban! as NSString).substring(with: NSRange(location: 4, length: details.bankCodeLength))
  ...
  open class func convertToIBAN(_ account: String, bankCode: String, countryCode: String,
                                validateAccount: Bool = true) -> [String: AnyObject] {
