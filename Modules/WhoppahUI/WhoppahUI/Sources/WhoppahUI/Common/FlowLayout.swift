...
struct FlowLayout<B, T: Hashable, V: View>: View {
  ...
  let items: [T]
  let viewMapping: (T, Bool) -> V

  @State private var totalHeight: CGFloat

  init(mode: Mode, binding: Binding<B>, items: [T], viewMapping: @escaping (T, Bool) -> V) {
    self.mode = mode
    _binding = binding
    self.items = items
  ...
  private func content(in g: GeometryProxy) -> some View {
    ...
    return ZStack(alignment: .topLeading) {
      ForEach(self.items, id: \.self) { item in
          self.viewMapping(item, item == self.items.last)
  ...
  static var previews: some View {
    FlowLayout(mode: .scrollable,
                               ...
                               items: ["Some long item here", "And then some longer one",
                                          ...
                                          "And then a very very very long one"]) { item, isLastItem in
      Text(item)
        .font(.system(size: 12))
