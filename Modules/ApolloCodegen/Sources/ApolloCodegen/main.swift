...
struct SwiftScript: ParsableCommand {
    ...
    static let queriesPath = "../graphql-mobile"
    static let endpoint = URL(string: "https://gateway.testing.whoppah.com")!
    static let targetPath = "WhoppahDataStore/Sources/WhoppahDataStore/Generated"
    static let cleanModelTargetPath = "WhoppahModel/Sources/WhoppahModel/Generated"
    ...
    struct DownloadSchema: ParsableCommand {
        ...
        func run() throws {
            ...
            let folderForDownloadedSchema = fileStructure.sourceRootURL
                .apollo.childFolderURL(folderName: SwiftScript.queriesPath)
            ...
            let schemaDownloadOptions = ApolloSchemaDownloadConfiguration(using: .introspection(endpointURL: SwiftScript.endpoint),
    ...
    struct GenerateCode: ParsableCommand {
        ...
        func cleanup(atURL url: URL) {
            let fileManager = FileManager.default
            guard let allFiles = try? fileManager.contentsOfDirectory(at: url, includingPropertiesForKeys: nil) else {
                return
            }
            var sqliteFiles = allFiles.filter { $0.pathExtension.elementsEqual("graphqls") }
            sqliteFiles.append(contentsOf: allFiles.filter { $0.pathExtension.elementsEqual("json") })
        ...
        func run() throws {
            ...
            let schemaURL = targetRootURL.appendingPathComponent("schema.graphqls")
            let targetURL = fileStructure.sourceRootURL.appendingPathComponent(SwiftScript.targetPath)
            let codegenOptions = ApolloCodegenOptions(mergeInFieldsFromFragmentSpreads: true,
                                                      namespace: "GraphQL",
                                                      outputFormat: .multipleFiles(inFolderAtURL: targetURL),
            ...
            let cleanModelTargetURL = fileStructure.sourceRootURL.appendingPathComponent(SwiftScript.cleanModelTargetPath)
            let cleanDataGenerator = CleanDataModelGenerator(schemaURL: schemaURL,
                                                             targetURL: cleanModelTargetURL)
            try cleanDataGenerator.generate()
            
            cleanup(atURL: targetRootURL)
        }
    }
